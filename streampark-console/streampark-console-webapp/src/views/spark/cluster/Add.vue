...
<script setup lang="ts" name="AddCluster">
  ...
  import { fetchCheckCluster, fetchCreateCluster } from '/@/api/spark/sparkCluster';
          ...
          if (resp) {
            await Swal.fire({
              ...
              title: values.clusterName.concat(
                t('setting.sparkCluster.operateMessage.createSparkSessionClusterSuccessful'),
              ),
            ...
            await Swal.fire(
              'Failed',
              t('setting.sparkCluster.operateMessage.createSparkSessionClusterFailed'),
              'error',
            );
          }
        } else {
    ...
    >
      <template #formFooter>
        <div class="flex items-center w-full justify-center">
          <a-button @click="go('/spark/cluster')">
            {{ t('common.cancelText') }}
          </a-button>
          <a-button
            id="e2e-sparkcluster-submit-btn"
            class="ml-4"
            :loading="getLoading"
            type="primary"
            @click="submit()"
